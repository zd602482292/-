...
Page({
    ...
    //读取公告列表数据库，初始状态读取11个，上拉触底读取更多
    getData(num=11,page=0){
      wx.showLoading({
        title: '加载中...',
      })
      db.collection('announcement').orderBy('time','desc').skip(page).limit(num)
      .where(
        {
    ...
    onReachBottom: function () {
      //上拉触底读取更新公告
     var page=this.data.gonggao.length
      this.getData(5,page)
    },
